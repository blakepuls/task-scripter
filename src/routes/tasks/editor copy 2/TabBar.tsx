...
export function TabBar({
  ...
  async function changeTab(tab: ITabMeta) {
    ...
    const updatedEditorConfig = {
      ...
      active_tab_index: tabs.findIndex((t) => t.path === tab.path),
      backwardHistory: [...(editorConfig.backwardHistory || []), curTab].filter(
        (tab): tab is ITabMeta => !!tab
      ),
      forwardHistory: editorConfig.forwardHistory || [],
    };
  ...
  async function removeTab(tab: ITabMeta) {
    ...
    const updatedTabs = tabs.filter((t) => t.path !== tab.path);
    setTabs(updatedTabs);
    const updatedEditorConfig = {
      ...(await getEditorConfig(task)),
      open_tabs: updatedTabs,
      active_tab_index: updatedTabs.length > 0 ? 0 : null,
    ...
    if (updatedTabs.length > 0) {
      changeTab(updatedTabs[0]); // Change to the first tab if there are tabs remaining
    }
  }
  ...
  useHotkeys(
    ...
    () => {
      ...
      if (lastTab) {
        const updatedEditorConfig = {
          ...editorConfig,
          backwardHistory: rest, // Just keep the rest of the history
          forwardHistory: [
            curTab,
